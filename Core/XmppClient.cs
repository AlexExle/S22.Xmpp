...
namespace S22.Xmpp.Client {
	...
	public class XmppClient : IDisposable {
        ...
        /// <remarks>Use this constructor if you wish to connect to an XMPP server using
        /// an existing set of user credentials.</remarks>
        public XmppClient(string hostname, string serverhostname, string username, string password,
            int port = 5222, bool tls = true, RemoteCertificateValidationCallback validate = null)
        {
            im = new XmppIm(hostname, serverhostname, username, password, port, tls, validate);
            // Initialize the various extension modules.
            LoadExtensions();
        }
